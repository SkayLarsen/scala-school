...
class Application(isTestEnv: Boolean) {
  ...
  val usefulService: UsefulService = if (isTestEnv)
    new SQLAPI("test db Resource") with TestServiceImpl //передайте "test db Resource" в качестве ресурса в конструктор SQLAPI
  else
    new SQLAPI("production Resource") with ProductionServiceImpl //передайте "production Resource" в качестве ресурса в конструктор SQLAPI
