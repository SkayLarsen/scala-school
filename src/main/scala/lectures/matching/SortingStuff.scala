...
object SortingStuff extends App {
  ...
  private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox) = item match {
    case watch@(item: Watches) if watch.cost > 1000 => stuffBox.copy(watches = watch :: stuffBox.watches)
    case boots@(item: Boots) if boots.brand == "Adidas" || boots.brand == "Converse" => stuffBox.copy(boots = boots :: stuffBox.boots)
    case book@(item: Book) if book.isInteresting => stuffBox.copy(books = book :: stuffBox.books)
